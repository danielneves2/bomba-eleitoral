...
export function createPixelAssets({geometries,materials,textures}) {
  ...
  return (name,size=1)=>{
    ...
    if(!mat) {
      ...
      (['steak','chair','sword'].includes(name)?loadCutout(`/item-${name}-pixel-v11.png`):new Promise((resolve,reject)=>{const img=new Image();img.onload=()=>resolve(img);img.onerror=reject;img.src=`/specials/${name}.svg`;})).then(canvas=>{if(!disposed){texture.image=canvas;texture.needsUpdate=true;}}).catch(error=>console.error(`Unable to load ${name} sprite`,error));
    }
